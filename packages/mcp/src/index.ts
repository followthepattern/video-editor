...
/**
 * MCP stdio server. Edits the same `project.json` the web UI/back end use, so
 * any change Claude makes appears live in an open browser (the backend watches
 * the file). Point both at the same project with VE_PROJECT_DIR.
 */

const projectDir = resolve(
  process.env.VE_PROJECT_DIR || join(process.cwd(), "projects/default"),
);
const projectFile = join(projectDir, "project.json");
const exportsDir = join(projectDir, "exports");

const store = await ProjectStore.open(projectFile);
...
server.registerTool(
  ...
  {
    title: "Get project",
    description: "Return the full project (tracks, clips, assets, resolution).",
    inputSchema: {},
  },
  async () => ok(store.get()),
);
...
server.registerTool(
  ...
  {
    title: "List media",
    description: "List imported media assets available to place on the timeline.",
    inputSchema: {},
  },
...
server.registerTool(
  ...
  async ({ trackId, content, start, duration, fontSize, color }) => {
    try {
      return ok(
        await store.addTextOverlay(trackId, { content, fontSize, color }, { start, duration }),
      );
    } catch (err) {
      return fail(String(err));
    }
...
server.registerTool(
  ...
  async () => {
    try {
      ...
      const out = join(exportsDir, `${newId("export")}.mp4`);
...
server.registerTool(
  "render_frame",
  {
    title: "Render frame",
    description:
      "Render the project and return a single composited frame (JPEG) at the given time, so you can see the result.",
...
console.error(`[mcp] video-editor server ready (project: ${projectFile})`);
